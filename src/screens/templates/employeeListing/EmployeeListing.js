...
const EmployeeListing = () => {
  ...
  const [filteredEmployeesList, setFilteredEmployeesList] = useState(employeesData);
  const itemsPerPage = 30;

  const handleFilter = (status) => {
    setStatusFilter(status);
    if (status === 'Online') {
      setFilteredEmployeesList(employeesData.filter(employee => employee.status === 'Online'));
    } else if (status === 'Offline') {
      setFilteredEmployeesList(employeesData.filter(employee => employee.status === 'Offline'));
    } else {
      setFilteredEmployeesList(employeesData); // Reset to all employees
    }
    ...
    <div className="employee-listing-container p-3">
      ...
      <Row>
        {viewMode === 'grid' ? (
              ...
              <Card className="employee-card mb-3">
                <Card.Body className="d-flex align-items-center">
                  ...
                  <div>
                    ...
                    <small className="employee-position">{employee.position}</small>
                  </div>
          ...
          <TableView employees={paginatedEmployees} />
        )}
      </Row>
